...
for (i in xs)
{
 ...
 #   t3=Sys.time()-ts;
    ts=Sys.time();
    a4=net_emd_fast(dhist1,dhist2,method='exhaustiveHalfNoOffset');
    t4=Sys.time()-ts;
    a0l<-c(a0l,a0)
    a1l<-c(a1l,a1)
    a2l<-c(a2l,a2)
...
points(xs,t4l,col='purple',lty = 4)
lines(xs,t4l,col='purple')
points(xs,t0l,col='black',lty = 4)
lines(xs,t0l,col='black')
legend("topleft",c("ExhaustfastVer1 ","OptFast","ExhaustFastVer2","NewExhaustive","Opt"),pch=c(1,1,1,1),  col=c("blue","red","green","purple","black"))
