...
dhist <- function(locations, masses, smoothing_window_width = 0, sorted = TRUE) {
  ...
  dhist <- list(
    locations = locations, masses = masses,
...
dhist_from_obs_slow <- function(observations) {
  ...
  counts <- sapply(locations, function(location) {
    sum(observations == location)
  })
...
dhist_ecmf <- function(dhist) {
  ...
  dhist_ecmf <- stats::approxfun(
    x = x_knots, y = cum_mass,
...
area_between_dhist_ecmfs <- function(dhist_ecmf1, dhist_ecmf2) {
  ...
  if (ecmf_type == "constant") {
    ...
    segment_areas[trapezium] <-
      segment_area_trapezium(
        x_diff = x_diff[trapezium],
