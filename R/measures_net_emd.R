...
#' two sets of network features. This is done by individually normalising the
#' distribution of each feature so that they have unit mass and unit variance.
#' Then the minimun EMD between the same pair of features (one for each
#' corresponding graph) is calculated by considering all possible translations
#' of the feature distributions. Finally the average over all features is
#' reported.
#' This is calculated as follows:
#'   1. Normalise each feature histogram to have unit mass and unit variance.
#'   2. For each feature, find the minimum EMD between each pair of histograms
#'      considering all possible  histogram translations.
#'   3. Take the average minimum EMD across all features.
#' @param graph_1 A network/graph object from the \code{igraph} package.
#' \code{graph_1} can be set to \code{NULL} (default) if \code{dhists_1} is
#' provided.
#' @param graph_2 A network/graph object from the \code{igraph} package.
#' \code{graph_2} can be set to \code{NULL} (default) if \code{dhists_2} is
#' provided.
#' @param dhists_1 Either, a \code{dhist} discrete histogram object, or list of
#' such objects, or a matrix of network features (each column representing a
#' feature). \code{dhists_1} can be set to \code{NULL} (default) if
#' \code{graph_1} is provided. A \code{dhist} object can be obtained from
#' \code{graph_features_to_histograms}.
#' @param dhists_2 Same as \code{dhists_1}.
#' @param method The method to be used to find the minimum EMD across all
#' potential offsets for each pair of histograms. Default is "optimise" to use
...
#' histograms at all offsets that are candidates for the minimal EMD at the cost
...
#' props_1 <- cbind(
#'   L.Spectra = eigen(Lapg_1)$values,
#'   NL.Spectra = eigen(NLapg_1)$values
#' )
#' props_2 <- cbind(
#'   L.Spectra = eigen(Lapg_2)$values,
#'   NL.Spectra = eigen(NLapg_2)$values
#' )
#'
#' # Use of smoothing window 1 is given for discrete integer distributions. If
#' # the network features are considered continuous variables
#' # smoothing_window_width equal to zero is recommended.
#' netemd_one_to_one(
#'   dhists_1 = props_1,
#'   dhists_2 = props_2,
...
#'
#' @export
netemd_one_to_one <- function(graph_1 = NULL,
                              graph_2 = NULL,
                              dhists_1 = NULL,
                              dhists_2 = NULL,
                              method = "optimise",
  ...
  ## ------------------------------------------------------------------------
  # Check arguments 2
  # If dhists_1 is a matrix of network features then transform them to dhist
  # objects.
  ...
  if (igraph::is.igraph(graph_1)) {
    if (!is.null(dhists_1)) {
      warning("dhists_1 will be calculated from graph_1.")
    }
    dhists_1 <- gdd(
      graph = graph_1, feature_type = feature_type,
  ...
  if (igraph::is.igraph(graph_2)) {
    if (!is.null(dhists_2)) {
      warning("dhists_2 will be calculated from graph_2.")
    }
    dhists_2 <- gdd(
      graph = graph_2, feature_type = feature_type,
      max_graphlet_size = max_graphlet_size,
      ego_neighbourhood_size = ego_neighbourhood_size
    )
  }
  ...
  # discrete histograms
  pair_of_dhist_lists <- all(
    purrr::map_lgl(dhists_1, is_dhist)
  ) &&
    all(purrr::map_lgl(dhists_2, is_dhist))
...
#' Distributions
#' @param graphs A list of network/graph objects from the \code{igraph} package.
#' \code{graphs} can be set to \code{NULL} (default) if \code{dhists} is
#' provided.
#' @param dhists A list whose elements contain either: A  list of \code{dhist}
...
#' @export
netemd_many_to_many <- function(graphs = NULL,
                                dhists = NULL,
                                method = "optimise",
                                smoothing_window_width = 0,
                                return_details = FALSE,
                                ...
                                max_graphlet_size = 5,
                                ego_neighbourhood_size = 0) {
  if (max_graphlet_size > 4 & mc.cores > 1) {
    print(paste(
      "This function will compute orbits of graphlets up to size 5 using ",
      mc.cores,
      " cores. Depending on the density and size of the graphs, this may lead ",
      "to a large compsumption of RAM."
    ))
  }
  ...
  if (!is.null(graphs) & is.null(dhists)) {
    if (!all((unlist(sapply(X = graphs, FUN = igraph::is.igraph))))) {
      stop(
        "Graphs need to be igraph graph objects, or a list of dhists network ",
  ...
  if (!is.null(dhists)) {
    ...
    if (all((unlist(sapply(
      X = dhists, FUN =
        function(l) {
          all((unlist(sapply(X = l, FUN = is_dhist))))
        }
    ))))) {
    ...
    if (is.null(which_imput_type)) {
      warning(
        "dhists does not conform to a Matrix or dhist class for all ",
        "elmenents/netwroks in the list."
      )
    }
  }
  ## ------------------------------------------------------------------------
  # Check arguments 2
  # If dhists is a list of matrices of network features then transform them to
  # dhist objects.
  if (which_imput_type == "Matrix") {
    dhists <- sapply(
      X = dhists,
      FUN = graph_features_to_histograms,
      simplify = FALSE
    )
  }
  ## ------------------------------------------------------------------------
  ...
  out <- purrr::simplify(
    parallel::mcmapply(function(index_a, index_b) {
      netemd_one_to_one(
        dhists_1 = dhists[[index_a]], dhists_2 = dhists[[index_b]],
        method = method, return_details = return_details,
        smoothing_window_width = smoothing_window_width
      )
    },
    comp_spec$index_a, comp_spec$index_b,
    SIMPLIFY = FALSE, mc.cores = mc.cores
    )
  )
  if (return_details) {
    ...
    colnames(min_emds) <- purrr::simplify(
      purrr::map(1:num_features, ~ paste("MinEMD_O", . - 1, sep = ""))
    )
    min_offsets <- matrix(
      purrr::simplify(purrr::map(out, ~ .$min_offsets)),
      ncol = num_features,
      byrow = TRUE
    )
    colnames(min_offsets) <- purrr::simplify(
      purrr::map(1:num_features, ~ paste("MinOffsets_O", . - 1, sep = ""))
    )
    min_offsets_std <- matrix(
      purrr::simplify(purrr::map(out, ~ .$min_offsets_std)),
      ncol = num_features,
      byrow = TRUE
    )
    colnames(min_offsets_std) <- purrr::simplify(
...
#' Internal function to compute the minimum Earth Mover's Distance between
#' standarized and translated histograms
#'
#' Calculates the minimum Earth Mover's Distance (EMD) between two
#' discrete histograms after normalising each histogram to unit mass and
#' variance.
#' This is calculated as follows:
#'   1. Normalise each histogram to have unit mass and unit variance
#'   2. Find the minimum EMD between the histograms
#' @param dhists_1 A \code{dhist} discrete histogram object or a list of such
#' objects
#' @param dhists_2 A \code{dhist} discrete histogram object or a list of such
#' objects
#' @param method The method to use to find the minimum EMD across all potential
#' offsets for each pair of histograms. Default is "optimise" to use
#' R's built-in \code{stats::optimise} method to efficiently find the offset
