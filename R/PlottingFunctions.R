...
#'
#' @param clustering_method Clustering method as allowed in the \code{pheatmap} function from the \code{pheatmap} package.
#'
...
netdis.plot <- function(netdislist,whatrow=c(1,2)[2],clustering_method="ward.D",main="Nedis"){
  ...
  g_NETDIScomp <- igraph::graph.edgelist(el = as.matrix(netdislist$comp_spec[,c("name_a","name_b")]),directed = FALSE)
  igraph::edge_attr(graph = g_NETDIScomp,name = attrname) <- netdislist$netdis[whatrow,]
  adjmat <- igraph::get.adjacency(graph = g_NETDIScomp,type = "both",attr = attrname,names = TRUE,sparse = FALSE)
...
netemd.plot <- function(netemdlist,clustering_method="ward.D",main="NetEmd"){
  attrname <- "NeEmdVals"
  g_NETEMDcomp <- igraph::graph.edgelist(el = as.matrix(netemdlist$comp_spec[,c("name_a","name_b")]),directed = FALSE)
  igraph::edge_attr(graph = g_NETEMDcomp,name = attrname) <- netemdlist$netemds
  adjmat <- igraph::get.adjacency(graph = g_NETEMDcomp,type = "both",attr = attrname,names = TRUE,sparse = FALSE)
  vnames <- rownames(adjmat)
