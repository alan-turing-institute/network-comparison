...
make_function <- function(x1, v1, w1, x2, v2, w2) {
  f1 <- function(x) {
    res1 <- rep(0, length(x))
    m1 <- x1[1] < x
    m2 <- x <= (x1[1] + w1)
...
get_val <- function(x1, v1, w1, x2, v2, w2) {
...
test_that("1 element at 0 vs many test Mixture", {
  for (w1 in (1:10) * 2)
  {
    ...
    x2 <- 1:w1
    v2 <- (1:w1) / w1
    f1 <- make_function(x1, v1, 1, x2, v2, 1)
    res2 <- integrate(f1, 0, w1 + 1, abs.tol = 0.000000001)[[1]]

    res1 <- netemd_smooth(x1, v1, 1, x2, v2, 1)
...
test_that("Old failure case 2", {
  ...
  f1 <- make_function(x1, v1, w1, x2, v2, w2)
  res1 <- integrate(
    f1, min(min(x1), min(x2)), max(max(x1), max(x2)) + max(w1, w2)
  )[[1]]
  res2 <- netemd_smooth(x1, v1, w1, x2, v2, w2)
