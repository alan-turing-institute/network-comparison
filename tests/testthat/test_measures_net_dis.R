...
test_that(test_message, {
  ...
  graphlet_tuple_counts_ego <- function(graphlet_counts_ego,
                                        max_graphlet_size) {
...
context(
  paste(
    "Measures Netdis: Ego-network density values match those for manually",
    "verified networks"
  )
)
...
context(
  paste(
    "Measures Netdis: Ego-network density-binned reference counts for manually",
    "verified networks"
  )
)
test_that(
  paste(
    "Ego-network 4-node density-binned reference counts match manually",
    "verified totals"
  ),
  {
    # Set up a small sample network with at least one ego-network that contains
...
context(
  "Measures Netdis: scale_graphlet_counts_ego for manually verified networks"
)
test_that("Ego-network 4-node graphlet counts match manually verified totals", {
  ...
  # Count graphlets in each ego network of the graph with neighbourhood sizes of
  # 1 and 2
  max_graphlet_size <- 4
  min_ego_edges <- 0
  min_ego_nodes <- 0

  # Use previously tested functions to generate ego networks and calcualte
  # graphlet counts.
  # ego nets
...
context(
  paste(
    "Measures Netdis: Ego-network density-binned counts for manually",
    "verified networks"
  )
)
test_that(
  paste(
    "density_binned_counts output matches manually verified totals with",
    "different scaling and aggregation functions"
  ),
  {
    # Set up a small sample network with at least one ego-network that contains
    # at least one of each graphlets
    ...
    # Set node and graphlet labels to use for row and col names in
    # expected counts
    node_labels <- igraph::V(graph)$name
    graphlet_labels <- c("G0", "G1", "G2", "G3", "G4", "G5", "G6", "G7", "G8")
...
test_that("netdis_exp_counts works for graphlets up to 4 nodes", {
  ...
  # Order 1 densities should be:
  # 0.6000000 0.5000000 1.0000000 0.8333333 1.0000000
  # 0.6666667 0.7000000 0.7000000 1.0000000 1.0000000
  # 2. Ego-networks of order 2
  ...
  # Order 2 densities should be:
  # 0.3333333 0.3809524 0.5000000 0.3571429 0.5000000
  # 0.4642857 0.4642857 0.5238095 0.6000000 0.6000000
