...
test_that(
  paste(
    "discrete_hist generates correct discrete histograms for random integer",
    "observations"
  ),
  {
...
context(
  "dhist: constructor, equality operator and as_* transformation functions"
)
test_that(
  paste(
    "dhist constuctor has correct locations and masses (default smoothing,",
    "unsorted)"
  ),
  {
...
test_that(
  paste(
    "dhist constuctor has correct locations and masses (specified smoothing,",
    "unsorted)"
  ),
  {
...
test_that("Non-identical dhists are NOT considered equal", {
  ...
  dhist2_one_location_mismatch$locations[3] <-
    dhist2_one_location_mismatch$locations[1] + 1
...
test_that(
  paste(
    "dhist_variance difference for smoothed and unsmoothed dhists is",
    "smoothing_window_width^2 / 12"
  ),
  {
    dhist <- dhist(
      locations <- c(7, 42, 1, 21, 101, 9),
      masses = c(15, 12, 16, 13, 11, 14)
    )
...
test_that(
  paste(
    "normalise_histogram_variance output has variance of 1 for random integer",
    "histograms"
  ),
  {
    ...
    rand_dhists <- replicate(
      num_hists,
      dhist(masses = rand_masses(), locations = rand_locations()),
      simplify = FALSE
    )

    smoothing_window_width <- 1
    rand_dhists_unsmoothed <- purrr::map(rand_dhists, as_unsmoothed_dhist)
    rand_dhists_smoothed <- purrr::map(
      rand_dhists,
      as_smoothed_dhist,
      smoothing_window_width = smoothing_window_width
    )
    ...
    actual_dhist_unsmoothed <- purrr::map(
      rand_dhists_unsmoothed,
      function(dhist) {
...
test_that(
  paste(
    "normalise_histogram_variance output has variance of 1 for normal",
    "histograms"
  ),
  {
    num_hists <- 5
...
test_that("sort_dhist works", {
  ...
  class(dhist1) <- "dhist"
  dhist2 <- list(
    locations = c(3, 0, -62, 7, 16, -58),
    masses = c(23, 24, 26, 22, 21, 25)
  )
  class(dhist2) <- "dhist"
...
test_that(
  paste(
    "dhist_ecmf returns correct step function when smoothing_window_width is",
    "zero"
  ),
  {
    dhist1 <- dhist(
      locations = c(1, 2, 4, 7, 11, 16, 22),
      masses = c(21, 22, 23, 27, 31, 36, 42)
    )
    ...
    extra_knots <- c(
      actual_knots1[1] - 1,
      actual_knots1[length(actual_knots1)] + 1
    )
...
test_that(
  paste(
    "area_between_dhist_ecmfs returns correct value for simple integer dhists"
  ),
  {
...
test_that(
  paste(
    "area_between_dhist_ecmfs returns correct value for non-integer normalised",
    "dhists"
  ),
  {
...
test_that("harmonise_dhist_locations works A", {
  ...
  expected <- list(
    dhist1 = dhist(
      masses = c(11, 12, 13, 0, 0, 0),
      locations = c(1, 3, 5, 2, 4, 6),
      smoothing_window_width = 1,
      sorted = FALSE
    ),
    dhist2 = dhist(
      masses = c(21, 22, 23, 0, 0, 0),
      locations = c(2, 4, 6, 1, 3, 5),
      smoothing_window_width = 1,
      sorted = FALSE
    )
  )
...
test_that("harmonise_dhist_locations works B", {
  ...
  expected <- list(
    dhist1 = dhist(
      masses = c(1, 1, 1, 0, 0),
      locations = c(1, 3, 5, 4, 6),
      smoothing_window_width = 1,
      sorted = FALSE
    ),
    dhist2 = dhist(
      masses = c(1, 1, 1, 0, 0),
      locations = c(4, 5, 6, 1, 3),
      smoothing_window_width = 1,
      sorted = FALSE
    )
  )
