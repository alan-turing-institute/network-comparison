...
dhistA_unsmoothed <- normalise_dhist_variance(dhistA_norm_mass)
dhistB_unsmoothed <- normalise_dhist_variance(dhistB_norm_mass)
dhistA_smoothed <- as_smoothed_dhist(dhistA_unsmoothed, smoothing_window_width = 1/sigmaA)
dhistB_smoothed <- as_smoothed_dhist(dhistB_unsmoothed, smoothing_window_width = 1/sigmaB)
fcA <- dhist_ecmf(dhistA_unsmoothed)
fcB <- dhist_ecmf(dhistB_unsmoothed)
flA <- dhist_ecmf(dhistA_smoothed)
flB <- dhist_ecmf(dhistB_smoothed)
...
xMin <- min(x)
xMax <- max(x)
yMax <- max(sum(dhistA_unsmoothed$masses), sum(dhistB_unsmoothed$masses))
...
res_unsmoothed <- net_emd(dhistA_pre_norm, dhistB_pre_norm, smoothing_window_width = 0, return_details = TRUE)
res_smoothed <- net_emd(dhistA_pre_norm, dhistB_pre_norm, smoothing_window_width = 1, return_details = TRUE)
