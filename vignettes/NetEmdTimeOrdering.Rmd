...
```{r}
# As the trade networks are considerable dense, this example first considers a
# small number of networks. This example may take some minutes to run.
...
```{r}
# This example may take more than a few minutes to run  (approx. 20 mins), and
# it is not necessary to run it for the upcoming examples as a larger set of
# counts has been already computed.
if (FALSE) {
  counts <- list()
  for (i in seq_len(wtnets)) {
    counts[[names(wtnets)[i]]] <- count_orbits_per_node(
      graph = wtnets[[i]], max_graphlet_size = 5
    )
  }
}
```
...
# Results
netemd_matrix <- cross_comp_to_matrix(
  measure = netemd_result$netemds,
  cross_comparison_spec = netemd_result$comp_spec
)
