...
ref_path <- file.path(
  system.file(file.path("extdata", "random"), package = "netdist"),
  "ER_1250_10_1"
)
...
netdis_result <- netdis_many_to_many(
  graphs = query_graphs,
  ref_graph = ref_graph,
...
netdis3_mat <- cross_comp_to_matrix(
  measure = res3, cross_comparison_spec = netdis_result$comp_spec
)
...
netemdlist <- netemd_many_to_many(
  graphs = query_graphs, smoothing_window_width = 0, mc.cores = 1
)
netemd_mat <- cross_comp_to_matrix(
  measure = netemdlist$netemds, cross_comparison_spec = netemdlist$comp_spec
)

netemdlist_smooth <- netemd_many_to_many(
  graphs = query_graphs, smoothing_window_width = 1, mc.cores = 1
)
netemd_mat_smooth <- cross_comp_to_matrix(
  measure = netemdlist_smooth$netemds,
  cross_comparison_spec = netemdlist_smooth$comp_spec
)

par(mfrow = c(1, 2))
cex <- 1

graph_netemd_dists_smooth <- as.dist(netemd_mat_smooth)
plot(phangorn::upgma(as.dist(graph_netemd_dists_smooth), method = "average"),
  use.edge.length = FALSE,
  edge.width = cex * 2,
...
graph_netemd_dists <- as.dist(netemd_mat)
plot(phangorn::upgma(as.dist(graph_netemd_dists), method = "average"),
  use.edge.length = FALSE,
  edge.width = cex * 2,
