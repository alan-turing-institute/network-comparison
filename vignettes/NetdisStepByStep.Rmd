...
## Count the number of nodes and the subgraphs  in ego-networks
...
In this vignette the complete steps are given for "Expectation via a gold-standard network". The other two variations follow similar (simpler) steps, and are shown in other vignettes.
...
To calculate the expected counts the ego-network, the counts of ego-networks of the gold-standard network need to be obtained first:
```{r}
...
Finally, the average scaled subgraph counts are obtained for each subgraph in all density bins (`scaled_subgraph_counts_ref`).
```{r}
...
After obtaining the average scaled subgraph counts, the subgraph counts of the query networks can be centred:
```{r}
...
After the counts are centred, the total count is computed for each subgraph:
```{r}
