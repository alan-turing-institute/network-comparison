...
This vignette explains variants of Netdis via global parameter changes of the Netdis call functions. For a more in depth view of the inner steps of Netdis please check ["Netdis: step by step"](NetdisStepByStep.html) and ["Netdis-GP: step by step"](NetdisGPStepByStep.html).
...
For this variant the a reference graph or gold-standard graph has to be given in `ref_graph`:
...
For this variant please set `ref_graph` to the desired constant $k$:
...
Change values with care as not all combinations may be possible. The following shows possible changes to the subgraph/graphlet size, the radius of the ego-networks considered, the minimum number of nodes an edges that are allowed in the ego-networks used in Netdis  and finally some parameters to modify the ego-network binning. The following are the default values:
```{r}
...
These values can be directly imputed into the shortcut Netdis function calls:
```{r,fig.align='center',fig.dim=c(7,7)}
